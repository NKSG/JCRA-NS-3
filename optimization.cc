 ...
 * Modified: Pengfei Cui
 * For JCAR simulation
 * May 10th 2013
 * First Step: Simulate 10 nodes static Ad-Hoc networks
 */
...
NS_LOG_COMPONENT_DEFINE ("Optimization Simulation");
		...
		const NqosWifiMacHelper &wifiMac, const YansWifiChannelHelper &wifiChannel)
{
	...
	mobility.SetMobilityModel ("ns3::RandomDirection2dMobilityModel",
			"Bounds", RectangleValue (Rectangle (0, 50, 0, 60)),
...
int main (int argc, char *argv[])
{
	...
	//Define output file name and debug log information

	std::ofstream outfile ("optimization.plt");
	// Debug comments
	...
	// Need to be mofied/add parameters
	CommandLine cmd;
	cmd.Parse (argc, argv);

	Gnuplot gnuplot = Gnuplot ("optimization.eps");

	for (uint32_t i = 0; i < modes.size (); i++)
	{
		std::cout << modes[i] << std::endl;
		Gnuplot2dDataset dataset (modes[i]);
		// Increase rss to get the plot curve
		for (double rss = -102.0; rss <= -80.0; rss += 0.5)
		{
			...
			// Add Channel parameters, such as the next line, parameters are ("ns3 model", attribute of the model, value of the attribute)
			wifiChannel.SetPropagationDelay ("ns3::ConstantSpeedPropagationDelayModel","Speed",DoubleValue(2.6e8));
